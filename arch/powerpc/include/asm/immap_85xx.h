...
struct rio_lp_serial {
	...
	struct rio_lp_serial_port	port[CONFIG_SRIO_PORT_MAX_NUM];
};
...
struct rio_phys_err {
	struct rio_phys_err_port	port[CONFIG_SRIO_PORT_MAX_NUM];
};
...
struct rio_implement {
	...
	struct rio_impl_port_spec	port[CONFIG_SRIO_PORT_MAX_NUM];
};
...
struct rio_atmu_win {
	struct rio_atmu_row	outbw[CONFIG_SRIO_OB_WIN_NUM];
	u8	res0[64];
	struct rio_atmu_riw	inbw[CONFIG_SRIO_IB_WIN_NUM];
};

struct rio_atmu {
	struct rio_atmu_win	port[CONFIG_SRIO_PORT_MAX_NUM];
};

#ifdef CONFIG_SYS_RMU
struct rio_msg {
...
#ifdef CONFIG_SYS_RMU
	u8	res5[8192];
	struct rio_msg	msg[CONFIG_SRIO_MSG_UNIT_NUM];
	u8	res6[512];
...
#define MPC85xx_PORBMSR_HA_SHIFT	16
	u32	porimBSCr;	/* POR I/O impedance status & control */
