...
static void qspi_enable_ddr_mode(struct fsl_qspi_priv *priv)
{
	...
	reg = qspi_read32(priv->flags, &priv->regs->mcr);
	/* Disable the module */
	qspi_write32(priv->flags, &priv->regs->mcr, reg | QSPI_MCR_MDIS_MASK);

	/* Set the Sampling Register for DDR */
	reg2 = qspi_read32(priv->flags, &priv->regs->smpr);
	reg2 &= ~QSPI_SMPR_DDRSMP_MASK;
	reg2 |= (2 << QSPI_SMPR_DDRSMP_SHIFT);
	qspi_write32(priv->flags, &priv->regs->smpr, reg2);
	...
	qspi_write32(priv->flags, &priv->regs->mcr, reg);
}
...
static void qspi_init_ahb_read(struct fsl_qspi_priv *priv)
{
	...
	qspi_write32(priv->flags, &priv->regs->buf0cr,
		     QSPI_BUFXCR_INVALID_MSTRID);
	qspi_write32(priv->flags, &priv->regs->buf1cr,
		     QSPI_BUFXCR_INVALID_MSTRID);
	qspi_write32(priv->flags, &priv->regs->buf2cr,
		     QSPI_BUFXCR_INVALID_MSTRID);
	qspi_write32(priv->flags, &priv->regs->buf3cr,
		     QSPI_BUF3CR_ALLMST_MASK |
		     (0x80 << QSPI_BUF3CR_ADATSZ_SHIFT));

	/* We only use the buffer3 */
	qspi_write32(priv->flags, &priv->regs->buf0ind, 0);
	qspi_write32(priv->flags, &priv->regs->buf1ind, 0);
	qspi_write32(priv->flags, &priv->regs->buf2ind, 0);

	/*
	 * Set the default lut sequence for AHB Read.
	 * Parallel mode is disabled.
	 */
	qspi_write32(priv->flags, &priv->regs->bfgencr,
...
static int fsl_qspi_child_pre_probe(struct udevice *dev)
{
	...
	struct fsl_qspi_priv *priv = dev_get_priv(dev->parent);
...
static int fsl_qspi_claim_bus(struct udevice *dev)
{
	...
	priv->cur_amba_base = priv->amba_base[0] +
		FSL_QSPI_FLASH_SIZE * priv->cur_slave_plat->cs;
