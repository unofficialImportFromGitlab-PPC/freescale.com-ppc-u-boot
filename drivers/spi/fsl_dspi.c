...
static int fsl_dspi_claim_bus(struct udevice *bus)
{
	...
	cpu_dspi_claim_bus(bus->seq, priv->cur_slave_plat->cs);

	/* configure transfer mode */
	fsl_dspi_cfg_ctar_mode(priv,  priv->cur_slave_plat->cs, priv->mode);

	/* configure active state of CSX */
	fsl_dspi_cfg_cs_active_state(priv, priv->cur_slave_plat->cs,
				     priv->mode);
...
static int fsl_dspi_release_bus(struct udevice *bus)
{
	...
	cpu_dspi_release_bus(bus->seq, priv->cur_slave_plat->cs);
