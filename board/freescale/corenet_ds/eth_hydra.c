...
/*
 * Given an alias, return a phandle for the node that the alias points to.
 * If the given node does not already have a phandle, then one is created for
 * it, using the value of 'phandle'.
 *
 * Therefore, 'phandle' must not be the value of an existing phandle.  The
 * easiest way to ensure that is to specify the return value of
 * fdt_alloc_phandle(). fdt_alloc_phandle() doesn't actually allocate a
 * phandle, it simply returns the next free phandle value.
*/
static u32 find_or_alloc_phandle(void *fdt, const char *alias, u32 phandle)
{
	const char *path;
	u32 ph = 0;
	...
	if (path) {
		...
		if (off > 0) {
			...
			ph = fdt_get_phandle(fdt, off);

			/*
			 * If there is no phandle, then create one using the
			 * phandle value passed to us, which must be unique.
			 */
			if ((ph == 0) || (ph == -1)) {
				ph = phandle;
				fdt_setprop_cell(fdt, off, "linux,phandle", ph);
			}
		}
	}
...
*/
static void fdt_set_phy_handle(void *fdt, char *compat, phys_addr_t addr,
			       const char *alias)
{
	int offset;
...
static void fdt_set_mdio_mux(void *fdt, const char *alias, u32 mux)
{
	...
	do_fixup_by_path(fdt, path, "fsl,hydra-mdio-muxval", &mux, sizeof(mux), 1);
}
