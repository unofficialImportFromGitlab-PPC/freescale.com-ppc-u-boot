...
{
	u8 dpm_rrp[5], ret[2];
	dpm_rrp[0] = RRP_OPCODE;
	/* POL is 0 */
	dpm_rrp[1] = 0;
	dpm_rrp[2] = r;
	i2c_write_read(I2C_DPM_ADDR, dpm_rrp, 3, ret, 2);
...
{
	u8 dpm_wrm[5], ret;
	dpm_wrm[0] = WRM_OPCODE;
	dpm_wrm[1] = r;
	dpm_wrm[2] = d;
	i2c_write_read(I2C_DPM_ADDR, dpm_wrm, 3, &ret, 1);
	if (ret == DPM_SUCCESS) { /* the DPM returned success as status */
...
int dpm_wrp(u8 r, u8 d)
{
	...
	dpm_wrp[5] = r;
	dpm_wrp[6] = d;
	i2c_write_read(I2C_DPM_ADDR, dpm_wrp, 7, &ret, 1);
	if (ret == DPM_SUCCESS) { /* the DPM returned success as status */
