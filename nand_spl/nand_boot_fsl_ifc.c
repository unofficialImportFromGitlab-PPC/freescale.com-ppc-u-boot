/*
 * NAND boot for Freescale Integrated Flash Controller, NAND Flash Control Machine
 *
...
static void nand_load(unsigned int offs, int uboot_size, uchar *dst)
{
	...
	while (pos < uboot_size) {
		...
		do {
			...
			if (port_size == 8) {
				if (i++ < 2 &&
					buf[sram_addr + page_size + bad_marker] != 0xff) {
					puts("skipping\n");
				...
				if (i++ < 2 &&
					buf[sram_addr + page_size + bad_marker] != 0xff &&
					buf[sram_addr + page_size + bad_marker + 1] != 0xff) {
					puts("skipping\n");
					offs = (offs + blk_size) & ~(blk_size - 1);
					pos &= ~(blk_size - 1);
					break;
